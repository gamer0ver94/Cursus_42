...
/*   Updated: 2022/09/01 15:53:15 by dpaulino         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	player_control(t_data *data, int key)
{
		...
		&& (data->menu.status == SELECTED))
	{
		...
		mlx_put_image_to_window(data->mlx, data->window.start,
			data->player.score_table.start, 80, 8);
		mlx_string_put(data->mlx, data->window.start, 100, 22, 3093151,
			ft_itoa(data->player.score));
		mlx_string_put(data->mlx, data->window.start, 15, 22,
			3093151, "SCORE");
	}
}
